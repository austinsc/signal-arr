...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('webpack', 'Webpack files.', function() {
    ...
    [].concat(options).forEach(function(opts) {
      opts.context = path.resolve(process.cwd(), opts.context);
      opts.output.path = path.resolve(process.cwd(), opts.output.path);
    });
    ...
    if (cache) {
      [].concat(options).forEach(function(o) {
        o.cache = false;
      });
    }
    ...
    function handler(err, stats) {
      ...
      if (typeof storeStatsTo === 'string') {
        ...
        grunt.file.write(storeStatsTo, JSON.stringify(stats.toJson()));
      }
